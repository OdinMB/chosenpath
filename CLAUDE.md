...
- `/.context/image-generation.md` - Image generation flows, models/settings (which env var drives which flow; the image model and quality defaults come from the owner's blind rating of 2026-09-24, so change them only on a new rating), and the image-model eval harness
