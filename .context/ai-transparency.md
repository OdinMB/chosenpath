...
Model ids are the code defaults: `server/src/shared/llm/textModelSettings.ts` for text, `server/src/config.ts` for images. Render environment variables may override them (ASK-OWNER, §11). A text override can only pick another OpenAI model of the gpt-4.x or gpt-6 family. Any other name stops the server at startup (`server/src/shared/llm/chatModel.ts`; `.context/text-model-eval.md`).

| # | Feature | Call site | Vendor · model id | Modality | Length | Mode | Destination | Human review | First public (F4) |
|---|---|---|---|---|---|---|---|---|---|
| 1 | Story narrator: beats, interludes, options, threads and switches, pregenerated beats, AI-assigned odds | `server/src/game/services/AIStoryGenerator.ts`; `prompts/BeatPromptService.ts` | OpenAI · `gpt-4.1-mini` (`TEXT_MODEL_NAME`, `SWITCH_THREAD_MODEL_NAME`; multiplayer may override with `MULTIPLAYER_TEXT_MODEL_NAME`, `MULTIPLAYER_SWITCH_THREAD_MODEL_NAME`, unset by default) | text | beats **≥200** (5–6 paragraphs, about 400–800 tokens); titles, options, interludes and odds <200 | conversational (turn-based choices) | the session's players; stored and replayable | none | ≤2025-05-24 |
| 2 | Custom-story setup from a free-text premise | `server/src/stories/StoryCreationService.ts`; `AIStoryGenerator.ts` | OpenAI · `gpt-4.1` (`SETUP_MODEL_NAME`, falling back to `GENERATION_MODEL_NAME`) | text | **≥200** in aggregate | one-shot form | creator and co-players; stored | none | ≤2025-05-24 |
