...
- State that only an interaction changes (a ticked checkbox) can't be rendered either. Drive it through what the page reads on load (the query string via a mocked `useSearchParams`), or assert it on the component that takes it as a prop. `ShareLink` reads `window.location` while rendering, so mock it on pages that show it.
...
- Mock `react-router-dom` hooks (`useNavigate`, `useSearchParams`, `useLoaderData`) instead of rendering `MemoryRouter`: react-router's types come from the root `@types/react` 19 and the client's from 18, so `<MemoryRouter>` fails the test type check.
