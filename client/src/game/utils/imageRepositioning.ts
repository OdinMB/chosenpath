...
export function optimizeImagePositions(
  ...
  let positions = currentPositions.filter(
...
export function enforceNoLastParagraph(
  ...
  const maxUsableParagraph = totalParagraphs - 1;
  const imagesInLastParagraph = positions.filter(
    (pos) => pos === totalParagraphs
  );
  ...
  for (let i = 0; i < imagesInLastParagraph.length; i++) {
    ...
    while (
      result.includes(nextAvailablePosition) &&
      nextAvailablePosition <= maxUsableParagraph
    ) {
  ...
  while (changed && passes < maxPasses) {
    ...
    for (let i = 0; i < result.length - 1; i++) {
      ...
      if (current !== undefined && next !== undefined && next - current < 2) {
        ...
        if (
          targetPosition <= maxUsableParagraph &&
          !result.includes(targetPosition)
        ) {
...
export function validateConstraints(
  ...
  const lastParagraphViolations = positions.filter(
    (pos) => pos > maxUsableParagraph
  );
  if (lastParagraphViolations.length > 0) {
    violations.push(
      `Images in last paragraph: ${lastParagraphViolations.join(", ")}`
    );
  }
